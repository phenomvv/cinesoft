...
export const fetchMovieDetails = async (id: string, type: 'movie' | 'show'): Promise<Movie | null> => {
  try {
    ...
    const data = await fetchTMDB(endpoint, { append_to_response: 'credits,watch/providers,videos,images', include_image_language: 'en,null' });
    ...
    // Find a logo image
    const logoImg = data.images?.logos?.find((l: any) => l.iso_639_1 === 'en') || data.images?.logos?.[0];
    const logoUrl = logoImg ? `${LOGO_BASE}${logoImg.file_path}` : undefined;

    let seasons: Season[] = [];
    if (type === 'show') { seasons = await fetchSeasons(id); }
    return { ...mapMovie(data), director, cast: castData, genres, streamingPlatforms, trailerUrl, seasons, logo: logoUrl };
  } catch (e) { return null; }
};
