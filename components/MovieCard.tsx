...
export const MovieCard = memo(({ 
  ...
  return (
    ...
    >
      ...
      {rank && (
        <div className="absolute -left-2 -bottom-3 z-20 pointer-events-none">
            <span 
                className="text-8xl font-black italic tracking-tighter text-[#050505] drop-shadow-xl"
      ...
      <div className="mt-2.5 px-1 relative z-30">
        <h3 className={`text-xs font-bold truncate text-gray-100 group-hover:text-[#6B46C1] transition-colors ${rank ? 'ml-6' : ''}`}>{movie.title}</h3>
        <p className={`text-[9px] text-gray-500 font-bold uppercase tracking-widest flex items-center gap-1 ${rank ? 'ml-6' : ''}`}>
