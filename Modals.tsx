...
const StarRating = memo(({ rating, onRate }: { rating: number, onRate: (n: number) => void }) => {
  return (
    ...
    >
      <div className="flex flex-col items-center justify-center py-3 px-4 rounded-2xl border border-white/10 bg-black/40 backdrop-blur-xl shadow-xl mb-6">
        ...
        <p className={`text-[8px] font-black uppercase tracking-[0.3em] mt-1.5 transition-colors ${rating > 0 ? 'text-yellow-500' : 'text-gray-500'}`}>
...
export const MovieDetailModal = memo(({ movie: initialMovie, onClose, user, setUser, onToggleWatchlist, onToggleWatched, onToggleFavorite, onRateMovie, onSelectPerson, onPlayTrailer, onShowToast }: any) => {
  ...
  const bgBlur = useTransform(scrollY, [0, 400], ["blur(0px)", "blur(10px)"]);
        ...
        >
           ...
           >
             ...
             <motion.div layout className="grid grid-cols-4 gap-3 mb-2">
              ...
              <motion.p layout className="text-sm font-medium leading-relaxed text-gray-300">{movie.description}</motion.p>
